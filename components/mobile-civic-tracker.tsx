...
export function MobileCivicTracker() {
  ...
  const [dashboardView, setDashboardView] = useState<"overview" | "ward" | "state" | "trends">("overview")
  const [expandedChart, setExpandedChart] = useState<string | null>(null)
  const [dateFilter, setDateFilter] = useState("30d")
  const [selectedWard, setSelectedWard] = useState("all")
  const [selectedState, setSelectedState] = useState("all")

  // Mock data matching public dashboard
  const userStatus = {
    submitted: 70,
    accepted: 52,
    resolved: 42,
  ...
  const wardData = [
    { ward: "Ward 1", issues: 45, resolved: 38, points: 420 },
    { ward: "Ward 2", issues: 32, resolved: 28, points: 310 },
    { ward: "Ward 3", issues: 28, resolved: 25, points: 285 },
    { ward: "Ward 4", issues: 22, resolved: 18, points: 220 },
    { ward: "Ward 5", issues: 18, resolved: 16, points: 180 },
  ]

  const stateData = [
    { state: "Tamil Nadu", issues: 1250, resolved: 1100, points: 12500 },
    { state: "Karnataka", issues: 980, resolved: 850, points: 9800 },
    { state: "Kerala", issues: 750, resolved: 680, points: 7500 },
    { state: "Andhra Pradesh", issues: 620, resolved: 520, points: 6200 },
    { state: "Telangana", issues: 580, resolved: 490, points: 5800 },
  ]

  const trendData = [
    { month: "Jan", submitted: 120, resolved: 95 },
    { month: "Feb", submitted: 135, resolved: 110 },
    { month: "Mar", submitted: 150, resolved: 125 },
    { month: "Apr", submitted: 165, resolved: 140 },
    { month: "May", submitted: 180, resolved: 155 },
    { month: "Jun", submitted: 195, resolved: 170 },
  ]
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      const response = await fetch("/api/feedback", {
        ...
        body: JSON.stringify({ feedback, source: "mobile_civic_tracker" }),
      })
  ...
  const renderExpandableChart = (chartId: string, title: string, data: any[], type: "bar" | "line" = "bar") => {
      ...
      <Card className="border-border">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={() => handleChartExpand(chartId)} className="h-6 w-6 p-0">
              {isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
            </Button>
          </div>
        </CardHeader>
        <CardContent className="pt-0">
          <div className={`transition-all duration-300 ${isExpanded ? "max-h-64" : "max-h-16"} overflow-hidden`}>
            {type === "bar" ? (
              <div className="space-y-2">
                {data.slice(0, isExpanded ? data.length : 3).map((item, index) => (
                  <div key={index} className="flex items-center justify-between text-xs">
                    <span className="truncate flex-1">{Object.values(item)[0]}</span>
                    <div className="flex items-center gap-2 flex-1">
                      <div className="flex-1 bg-muted rounded-full h-2">
                        <div
                          className="bg-primary h-2 rounded-full transition-all duration-500"
                          style={{
                            width: `${Math.min(((Object.values(item)[1] as number) / Math.max(...data.map((d) => Object.values(d)[1] as number))) * 100, 100)}%`,
                          }}
                        />
                      </div>
                      <span className="text-muted-foreground w-8 text-right">{Object.values(item)[1]}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {data.map((item, index) => (
                  <div key={index} className="flex flex-col items-center flex-1">
                    <div
                      ...
                      style={{
                        height: `${((Object.values(item)[1] as number) / Math.max(...data.map((d) => Object.values(d)[1] as number))) * 100}%`,
                      }}
                    />
                    <span className="text-xs text-muted-foreground mt-1">{Object.values(item)[0]}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="bg-[#0B3D91] text-white p-4 border-b-4 border-[#FF9933]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs opacity-90">Public Dashboard</p>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex gap-1">
                ...
                <Button
                  variant={dashboardView === "ward" ? "default" : "outline"}
                  size="sm"
                  onClick={() => setDashboardView("ward")}
                  className="text-xs px-2 py-1 h-7"
                >
                  Wards
                </Button>
                <Button
                  variant={dashboardView === "state" ? "default" : "outline"}
                  size="sm"
                  onClick={() => setDashboardView("state")}
                  className="text-xs px-2 py-1 h-7"
                >
                  States
                </Button>
                <Button
                  variant={dashboardView === "trends" ? "default" : "outline"}
                  size="sm"
            ...
            {dashboardView === "overview" && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Card className="border-border">
                    <CardContent className="p-3 text-center">
                      <div className="text-lg font-bold text-[#138808]">125</div>
                      <div className="text-xs text-muted-foreground">Resolved</div>
                    </CardContent>
                  </Card>
                  <Card className="border-border">
                    <CardContent className="p-3 text-center">
                      <div className="text-lg font-bold text-[#FF9933]">86%</div>
            ...
            {dashboardView === "ward" && (
              <div className="space-y-4">
                <Card className="border-border">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg flex items-center gap-2">
                      ...
                      Ward-wise Civic Points
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {wardData.map((ward, index) => (
                        <div key={ward.ward} className="border-b border-border pb-3 last:border-b-0">
                          <div className="flex items-center justify-between mb-2">
                            <span className="font-medium text-sm">{ward.ward}</span>
                            <Badge variant="secondary">{ward.points} pts</Badge>
                          </div>
                          ...
                          <div className="mt-2 bg-muted rounded-full h-2">
                            <div
                              className="bg-primary h-2 rounded-full transition-all duration-500"
                              style={{ width: `${(ward.resolved / ward.issues) * 100}%` }}
                            />
                          </div>
                        </div>
                      ))}
                ...
                {renderExpandableChart(
                  "ward-chart",
                  "Ward Performance",
                  wardData.map((w) => ({ name: w.ward, value: w.points })),
                )}
              </div>
            )}
            ...
            {dashboardView === "state" && (
              <div className="space-y-4">
                <Card className="border-border">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg flex items-center gap-2">
                      <MapIcon className="w-5 h-5 text-[#138808]" />
                      State-wise Civic Points
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {stateData.map((state, index) => (
                        <div key={state.state} className="border-b border-border pb-3 last:border-b-0">
                          <div className="flex items-center justify-between mb-2">
                            <span className="font-medium text-sm">{state.state}</span>
                            <Badge variant="secondary">{state.points} pts</Badge>
                          </div>
                          <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground">
                            <div>Issues: {state.issues}</div>
                            <div>Resolved: {state.resolved}</div>
                          </div>
                          <div className="mt-2 bg-muted rounded-full h-2">
                            <div
                              className="bg-[#138808] h-2 rounded-full transition-all duration-500"
                              style={{ width: `${(state.resolved / state.issues) * 100}%` }}
                            />
                          </div>
                        </div>
                      ))}
                ...
                {renderExpandableChart(
                  "state-chart",
                  "State Performance",
                  stateData.map((s) => ({ name: s.state, value: s.points })),
                )}
              </div>
            )}
            ...
            {dashboardView === "trends" && (
              <div className="space-y-4">
                <Card className="border-border">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="text-center">
                        <div className="text-2xl font-bold text-[#138808]">-15%</div>
                ...
                {renderExpandableChart(
                  "trend-submitted",
                  "Issues Submitted",
                  trendData.map((t) => ({ month: t.month, value: t.submitted })),
                  "line",
                )}
                {renderExpandableChart(
                  "trend-resolved",
                  "Issues Resolved",
                  trendData.map((t) => ({ month: t.month, value: t.resolved })),
                  "line",
                )}
                        ...
                        <div key={index} className="flex flex-col items-center flex-1">
                          <div className="flex flex-col items-center w-full gap-1">
                            <div
                              className="bg-primary rounded-t w-full transition-all duration-500"
                              style={{
                                height: `${(item.submitted / Math.max(...trendData.map((d) => d.submitted))) * 60}px`,
                              }}
                            />
                            <div
                              className="bg-[#138808] rounded-t w-full transition-all duration-500"
                              style={{
                                height: `${(item.resolved / Math.max(...trendData.map((d) => d.resolved))) * 60}px`,
                              }}
                            />
                          </div>
            ...
            <div className="space-y-3">
              <Card className="border-border">
                <CardContent className="p-4">
                  <h4 className="font-medium">Clean Coimbatore</h4>
                  <p className="text-sm text-muted-foreground mb-2">Community focused on cleanliness</p>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">1,234 members</span>
              ...
              <Card className="border-border">
                <CardContent className="p-4">
                  <h4 className="font-medium">Road Safety TN</h4>
                  <p className="text-sm text-muted-foreground mb-2">Improving road infrastructure</p>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">1,856 members</span>
                    <Button size="sm" variant="outline" className="h-7 text-xs bg-transparent">
                      Join
                    </Button>
                  </div>
